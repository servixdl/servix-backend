...
transbankRouter.post('/crear', async (req, res) => {
  ...
  const returnUrl = 'http://localhost:3000/webpay/retorno'; // URL que recibirá el resultado
 
  try {const ventaProvisional = {
      buyOrder,
    ...
    const ventap = await transactionModelos.createProvisionalSale(ventaProvisional);
     console.log(ventap)
...
if (!token) {
  ...
  try {
    ...
    const {
      ...
      hora_inicio,
      hora_termino,
      estado }= ventaProvisional
    const id_venta = await salesModelos.create({usuario_id,servicio_id,fecha_venta,total})
    const id_cita = await appointmentModel.create({id_venta,fecha_cita,hora_inicio,hora_termino,usuario_id,estado})

    res.redirect(`http://localhost:5173/salePay?venta_id=${id_venta}&cita_id=${id_cita}`)
  } catch (error) {
    console.error(error);
