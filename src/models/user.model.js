...
const register = async (user)=>{
    try{
        let {rut,nombre,fecha_nacimiento,correo,contrasena,tipo_usuario,profesion,direccion} = user;
    const passwordEncriptado = bcrypt.hashSync(contrasena,10);
    const values =[rut,nombre,fecha_nacimiento,correo,passwordEncriptado,tipo_usuario,profesion,direccion];
...
const getAll = async ()=>{
    try {
        const query = 'SELECT rut ,nombre,fecha_nacimiento,correo,tipo_usuario,profesion,direccion FROM usuario'
...
const update = async (rut, usuario) =>{
    try {
    const { nombre, fecha_nacimiento, correo, contrasena,tipo_usuario,profesion,direccion } = usuario;
    const result = await pool.query(
      ...
      [nombre, fecha_nacimiento, correo, contrasena,tipo_usuario,profesion,direccion, rut]
    );
...
const searchById = async(rut)=>{
    try {
        const query = 'SELECT rut,nombre,fecha_nacimiento,correo,tipo_usuario,profesion,direccion FROM usuario WHERE rut = $1';
...
export const usermodel = {register,login,searchById,destroy,getAll,update}