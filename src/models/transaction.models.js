...
const searchSalesAndAppointments = async(rut)=>{
    try {
        const query = `SELECT * FROM ventas v JOIN citas c on v.id_venta = c.venta_id  join servicios s on v.servicio_id = s.id_servicio where v.usuario_id = $1`
        const result = await pool.query(query,[rut])
        return result.rows
    } catch (error) {
        console.log("error al buscar ventas por usuario",error)
        throw error
    }
}
