...
const getBitacoraByUsuario = async (rut_usuario) => {
 const query =`
 SELECT s.nombre AS servicio, ts.tipo_servicio AS tipo,
 s.descripcion, c.fecha_cita AS fecha, v.total,
 c.id_cita, c.estado
 FROM citas c
...
const cancelarCita = async (id_cita) => {
 const query = "UPDATE citas SET estado = 'cancelada' WHERE id_cita = $1 RETURNING *";
